...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    for(int k=-TAP_RADIUS;k<=TAP_RADIUS;++k){
        ...
        float yp=(-R_V*uVLenFactor)*cos(tu),m=pow(smoothstep(FLARE_HEIGHT,0.0,abs(yp)),FLARE_EXP),wx=(1.0+FLARE_AMOUNT*m)*uVBeamWidth;
        vec2 sig=vec2(wx,1.0),p=vec2(0.0,yp);
        float mask=step(0.0,yp); // Restore one-sidedness
        b+=wt*bsa(uvc,p,mask*env,sig);
    }
    float sPix=clamp(yPix/(R_V*uVLenFactor),0.0,1.0),topA=pow(1.0-smoothstep(0.8,1.0,sPix),TOP_FADE_EXP);
    float L=a+b*topA;
...
export const LaserFlow: React.FC<LaserFlowProps> = ({
    ...
    useEffect(() => {
        ...
        const uniforms: any = {
            ...
            uVBeamWidth: { value: verticalBeamWidth }
        };
    ...
    useEffect(() => {
        ...
        uniforms.uVBeamWidth.value = verticalBeamWidth;

        const { r, g, b } = hexToRGB(color || '#d42828ff');
